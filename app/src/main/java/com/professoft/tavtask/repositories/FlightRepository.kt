...
import com.professoft.tavtask.utils.AirlinesResponse
...
class FlightRepository private constructor() {
    ...
    private lateinit var mAirlineCall: Call<AirlinesResponse>

    fun getArrivvalFlights(callback: NetworkResponseCallback, arr_icao: String): MutableLiveData<FlightsResponse> {
        mutableCallback = callback

        mFlightCall = FlightRestClient.getInstance().getFlightApiService().getArrivalFlights(Keys.apiKey(),arr_icao,"10")
    ...
    fun getDepartureFlights(callback: NetworkResponseCallback, dep_icao: String): MutableLiveData<FlightsResponse> {
        mutableCallback = callback

        mFlightCall = FlightRestClient.getInstance().getFlightApiService().getDepartureFlights(Keys.apiKey(),dep_icao,"10")
        mFlightCall.enqueue(object : Callback<FlightsResponse> {
